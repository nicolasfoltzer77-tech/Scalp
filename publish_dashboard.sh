cat >/opt/scalp/publish_dashboard.sh <<'SH'
#!/usr/bin/env bash
set -euo pipefail
...
# Si un token est fourni, on réécrit l'URL origin en https avec auth (en local seulement)
if [[ -n "${GIT_TOKEN:-}" && -n "${GIT_REPO:-}" ]]; then
  git -C "$REPO" remote set-url origin \
    "https://${GIT_TOKEN}@github.com/${GIT_REPO}.git"
fi

# On n’aspire QUE main (fetch minimal)
...
# Restaure le stash si besoin (après rebase)
if [[ -n "$STASH_REF" ]]; then
...
# Docs/ index.html (GitHub Pages via /docs)
echo "[publish] copy -> docs/index.html"
mkdir -p "$DOCS_DIR"
cp -f "$DASH" "$DOCS_DIR/index.html"

# Assure que les outputs générés ne re-déclenchent pas des conflits ultérieurs
grep -qxF "dashboard.html" "$REPO/.gitignore" || echo "dashboard.html" >> "$REPO/.gitignore"

# ==== Commit si changements, push ============================================
if ! git -C "$REPO" diff --quiet || ! git -C "$REPO" diff --cached --quiet; then
...
echo "[publish] done."
SH
chmod +x /opt/scalp/publish_dashboard.sh
