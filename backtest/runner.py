...
def default_loader(fetch_sync: Callable) -> Callable:
    # wrappe un fetch OHLCV sync (ou un cache local)
    return fetch_sync

class BacktestRunner:
    def __init__(self, loader, out_dir: str, strategy: str, cfg: Dict, cash: float, risk_pct: float, max_conc: int=4):
        self.loader = loader
    ...
    async def _run_one(self, symbol: str, timeframe: str, start: int, end: int) -> Dict:
        async with self.sem:
            engine = BacktestEngine(self.loader, self.strategy_fn, self.cfg, os.path.join(self.out_dir, f"{symbol}_{timeframe}"), self.cash, self.risk_pct)
            # engine.run_pair est sync; exécute dans thread pour ne pas bloquer
    ...
    async def run_all(self, symbols: List[str], timeframes: List[str], start: int, end: int) -> Dict:
        ...
        proposal = {
            "per_symbol_best": {k: {"timeframe": v["timeframe"], "score": v["score"], "pf": v["pf"], "winrate": v["winrate"], "maxdd": v["maxdd"]} for k,v in per_symbol_best.items()},
            "top_overall": top_overall,
            "suggested_timeframes": {k: v["timeframe"] for k,v in per_symbol_best.items()},
            "note": "Suggestion basée sur le score composite (WinRate, PF, MaxDD, Sharpe).",
        }
