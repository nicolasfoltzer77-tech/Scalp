...
def setup_logger(logs_dir: str) -> logging.Logger:
    ...
    log = logging.getLogger("backtest"); log.setLevel(logging.INFO); log.handlers.clear()
    fh = logging.FileHandler(p); sh = logging.StreamHandler(sys.stdout)
    fmt = logging.Formatter("%(asctime)s [%(levelname)s] %(message)s"); fh.setFormatter(fmt); sh.setFormatter(fmt)
    log.addHandler(fh); log.addHandler(sh); return log
...
def main():
    ...
    ap.add_argument("--schema-json", type=str, default=None)
    ap.add_argument("--schema-backtest", type=str, default=None)
    ap.add_argument("--schema-entries", type=str, default=None)
    args = ap.parse_args()

    cfg = load_yaml(args.config); rt = cfg.get("runtime", {})
    ...
    # --- charger schémas (auto-fallback vers schemas/) ---
    ...
    # si rien n'est passé, load_combined_or_split utilisera schemas/ par défaut
    ...
    for pair in assets:
        for tf in entry_tfs:
            try: df_entry = load_csv(data_dir, pair, tf, backfill_limit)
            except Exception as e: log.warning(f"[{pair}:{tf}] CSV KO: {e}"); continue
            ...
            out["strategies"][key] = {
                ...
                "created_at": now_ts, "expires_at": None, "expired": False,
                "params": {"tf": tf, "dir_tfs": dir_tfs},
                "metrics": {"pf": round(m["pf"],4), "mdd": round(m["mdd"],4), "trades": int(m["trades"]),
                            "wr": round(m["wr"],4), "sharpe": round(m["sharpe"],4)}
            }
    ...
    with open(os.path.join(reports_dir, "summary.json"), "w", encoding="utf-8") as f:
        json.dump({"generated_at": now_ts, "risk_mode": risk_mode, "rows": table_rows, "selected": sorted(out["strategies"].keys())}, f, indent=2, ensure_ascii=False)

    log.info("Écrit: strategies.yml.next + summary.json")
