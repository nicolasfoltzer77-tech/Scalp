...
def _parse_kv(text: str) -> Dict[str, str]:
    # ex: "symbols=BTCUSDT,ETHUSDT tf=1m,5m cash=12000 risk=0.007 data=data"
    out: Dict[str, str] = {}
...
def parse_backtest_args(cmd_tail: str, defaults: Dict[str, object]) -> BacktestArgs:
    ...
    symbols = (kv.get("symbols") or kv.get("s") or ",".join(defaults.get("top_symbols", []) or [])).strip()
    ...
    return BacktestArgs(symbols=sym_list, timeframes=tf_list, data_dir=data_dir, out_dir=out_dir,
                        cash=cash, risk=risk, slippage_bps=slip)
...
async def handle_backtest_command(
    *,
    ...
    Exemple d'appel :
      /backtest symbols=BTCUSDT,ETHUSDT tf=1m,5m cash=12000 risk=0.005 data=data out=result/backtests
    """
    args = parse_backtest_args(cmd_tail, runtime_config or {})
    if not args.symbols or not args.timeframes:
        await notifier.send("⚠️ Usage: /backtest symbols=BTCUSDT,ETHUSDT tf=1m,5m [cash=10000 risk=0.005 slippage_bps=2 data=data out=result/backtests]")
        return

    await notifier.send(f"🧪 Backtest en cours…\n"
    ...
    # Exécuter en thread bloquant (pandas/IO), pour ne pas figer l’event loop
    ...
    if summary_path.exists():
        ...
        for _, r in top.iterrows():
            lines.append(f"• {r['symbol']} {r['timeframe']} — ret {r['return_pct']:.2f}% | trades {int(r['n_trades'])} | win {r['win_rate_pct']:.1f}% | DD {r['max_dd_pct']:.1f}%")
        await notifier.send("\n".join(lines))
        # pièce jointe
        if hasattr(notifier, "send_document"):
            try:
                ...
                await notifier.send(f"(info) Envoi du fichier summary.csv impossible: {e}")
    else:
