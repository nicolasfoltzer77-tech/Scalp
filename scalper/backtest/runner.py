...
# --------- Petit moteur PnL ---------
def _bps(x: float) -> float: return x / 10_000.0
...
def simulate_symbol(ohlcv: List[List[float]], cfg: BTCfg, signal_fn: Callable) -> Tuple[List[Tuple[int,float]], Dict]:
    ...
    for i in range(1, len(ohlcv)):
        ...
        if signal == "BUY":
            ...
            target_pos = max(0.0, notional / price)
        elif signal == "SELL":
            notional = equity * cfg.risk_pct * strength
            target_pos = - max(0.0, notional / price)
...
# Certains fichiers live importaient: BTConfig, run_single, save_results
BTConfig = BTCfg  # alias

async def run_single(cfg: BTCfg, exchange, symbol: str | None = None) -> Dict:
    """Compat : lance un backtest mono-symbole en réutilisant run_multi."""
    if symbol:
        cfg = replace(cfg, symbols=[symbol])
    return await run_multi(cfg, exchange)

def save_results(res: Dict) -> Dict:
    """Compat no-op : les fichiers sont déjà écrits par run_multi."""
    return res
