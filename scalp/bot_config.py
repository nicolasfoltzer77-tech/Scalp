...
def fetch_pairs_from_mexc(base_url: str | None = None) -> List[str]:
    ...
    for each contract symbol. We now return every symbol provided by the API
    (still excluding BTC and ETH) so that callers can see the full list instead
    of filtering to only zero-fee markets.
    """

    pairs_with_fees = fetch_pairs_with_fees_from_mexc(base_url)

    pairs = [sym for sym, _maker, _taker in pairs_with_fees]
    filtered = [p for p in pairs if _base(p) not in {"BTC", "ETH"}]
    print(f"Pairs: {filtered}")
    return filtered
