...
class MexcFuturesClient:
    """Client REST pour l'API Futures de MEXC."""
    ...
    def _private_request(
        ...
        if method in ("GET", "DELETE"):
            ...
            r = requests.request(
                method, url, params=params, headers=headers, timeout=20
            )
        elif method == "POST":
            ...
            headers = self._headers(sig, req_ms)
            r = requests.post(
                url, data=body_str.encode("utf-8"), headers=headers, timeout=20
            )
        else:
    ...
    def place_order(
        ...
        """Placer un ordre sur MEXC Futures."""
        if self.paper_trade:
            logging.info(
