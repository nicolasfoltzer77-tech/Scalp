...
SELECT
    ...
    score_A
FROM ctx_A_latest
/* v_ctx_latest(instId,ctx,score_A) */;
...
/* v_signals_latest(instId,side,setup,ctx_A,score_A,score_B,ts_signal,status,ts) */;
...
/* DB: /opt/scalp/project/data/h.db */
-- SCHEMA ONLY --
...
SELECT 
    ...
    score_A,
    score_B,
    reason,         -- ✅ déclencheur visible
    ts_signal
FROM signals
ORDER BY ts_signal DESC
/* v_signals_last(instId,side,ctx,score_U,score_A,score_B,reason,ts_signal) */;
CREATE TABLE ctx_cache (
    instId TEXT PRIMARY KEY,
    ctx TEXT,
    score_U REAL,
    score_A REAL,
...
CREATE INDEX idx_ticks_ts ON ticks(ts_ms);
CREATE VIEW v_ticks_latest AS
SELECT
  instId,
  price AS lastPr,
  ts_ms,
  (strftime('%s','now')*1000 - ts_ms) AS age_ms,
  ROUND((strftime('%s','now')*1000 - ts_ms)/1000.0, 2) AS age_s,
  printf('%02d:%02d:%02d',
         ((strftime('%s','now') - ts_ms/1000) / 3600) % 24,
         ((strftime('%s','now') - ts_ms/1000) / 60) % 60,
         (strftime('%s','now') - ts_ms/1000) % 60
  ) AS age_hms
FROM ticks
ORDER BY ts_ms DESC
/* v_ticks_latest(instId,lastPr,ts_ms,age_ms,age_s,age_hms) */;
...
CREATE TABLE positions_closed(
        ...
        , qty REAL DEFAULT 0, reason_entry TEXT, reason_exit TEXT, ts_entry INTEGER DEFAULT 0);
CREATE TABLE sqlite_sequence(name,seq);
CREATE VIEW v_trades_history AS
SELECT
    instId,
    side,
    entry,
    ...
    reason_entry,
    reason_exit,
    (ts_close - ts_open)/1000 AS duration_s
FROM positions_closed
ORDER BY ts_close DESC
/* v_trades_history(instId,side,entry,exit,qty,pnl,ts_open,ts_close,reason_entry,reason_exit,duration_s) */;
...
CREATE TABLE positions_open (
    ...
    reason       TEXT,
    ts_open      INTEGER
, usd_size REAL, break_even_set INTEGER DEFAULT 0, pyramided INTEGER DEFAULT 0, BE INTEGER DEFAULT 0, PYR INTEGER DEFAULT 0, reason_entry TEXT, ts_entry INTEGER DEFAULT 0);
