...
SELECT 
  f30.instId,
  f30.ts AS ts,
  COALESCE((ABS(f5.macd_hist) + ABS(f15.macd_hist) + ABS(f30.macd_hist)) / 3.0, 0) AS score_A,
  CASE
    WHEN f5.macd > 0 AND f15.macd > 0 AND f30.macd > 0 AND f5.rsi14 > 55 THEN 'bullish'
    WHEN f5.macd < 0 AND f15.macd < 0 AND f30.macd < 0 AND f5.rsi14 < 45 THEN 'bearish'
    ELSE 'range'
  END AS ctx
FROM oa_30m f30
LEFT JOIN oa_15m f15 ON f30.instId = f15.instId
LEFT JOIN oa_5m  f5  ON f30.instId = f5.instId
GROUP BY f30.instId;
