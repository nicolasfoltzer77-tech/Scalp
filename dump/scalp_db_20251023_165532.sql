 ...
 * Exported: 2025-10-23 14:55:33 UTC
 ***************************************************************************/
...
/* v_x_pnl(id,instId,side,entry,exit,qty,leverage,ts_open,ts_close,pnl_usdt,pnl_pct,reason_exit,score_H,score_U,score_A,score_B,reason,pnl_brut,fees_total,pnl_net) */;
...
CREATE TABLE positions_closed (
  ...
  reason_exit TEXT,
  score_H     REAL DEFAULT 0.0,
  score_U     REAL DEFAULT 0.0,
  score_A     REAL DEFAULT 0.0,
  score_B     REAL DEFAULT 0.0,
  reason      TEXT DEFAULT '',
  pnl_brut    REAL DEFAULT 0.0,
...
SELECT
  'OPEN' AS state,
  order_id, instId, side, entry, sl, tp, qty, leverage, ts_open,
  upnl_latent AS pnl, reason
FROM positions_open
UNION ALL
SELECT
  'CLOSED' AS state,
  id AS order_id, instId, side, entry, NULL, NULL, qty, leverage, ts_open,
  pnl_usdt AS pnl, reason_exit
FROM positions_closed
ORDER BY ts_open DESC
/* v_positions(state,order_id,instId,side,entry,sl,tp,qty,leverage,ts_open,pnl,reason) */;
...
SELECT
  (SELECT value FROM account_state WHERE key='balance_usdt') AS balance,
  (SELECT value FROM account_state WHERE key='equity_usdt')  AS equity,
  (SELECT value FROM account_state WHERE key='upnl_latent')  AS upnl_latent,
  (SELECT value FROM account_state WHERE key='pnl_closed')   AS pnl_closed,
  datetime(MAX(updated_ts),'unixepoch','localtime') AS updated_local
FROM account_state
/* v_account(balance,equity,upnl_latent,pnl_closed,updated_local) */;
...
SELECT
  ...
  pnl_pct,
  reason_exit,
  datetime(ts_open/1000,'unixepoch') AS opened_at,
  datetime(ts_close/1000,'unixepoch') AS closed_at
FROM positions_closed
ORDER BY ts_close DESC
/* v_trades(instId,side,entry,exit,qty,leverage,pnl_usdt,pnl_pct,reason_exit,opened_at,closed_at) */;
...
CREATE TABLE positions_open (
    ...
    tp           REAL NOT NULL,
    qty          REAL NOT NULL,
    leverage     INTEGER NOT NULL,       -- lev FULL arrondi entier
