...
class Orchestrator:
    ...
    async def _step(self) -> None:
        # 1) mise à jour état (CSV + strategies.yml)
        self.state.refresh()

        # 2) affichage
        self.health.render()

        # 3) AUTO (refresh/backtest/promote)
        if self.cfg.auto:
            self.state.auto_actions(limit=self.cfg.limit, cooldown=self.cfg.cooldown_secs)

        # 4) signaux sur cases vertes (placeholder observe‑only)
        for (s, tf) in self.state.ready_pairs():
            # TODO: brancher evaluate_tick(exchange, s, tf, strat, exec_enabled)
            pass