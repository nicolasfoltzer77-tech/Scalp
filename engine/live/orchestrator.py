...
class RunConfig:
    ...
    timeframe: str = "1m"
    refresh_secs: int = 5
    cache_dir: str = "/notebooks/scalp_data/data"  # fallback sûr hors repo
...
async def _fetch_ohlcv_any(exchange: Any, symbol: str, timeframe: str, limit: int = 150) -> List[List[float]]:
    ...
    if callable(fetch):
        try:
            data = await fetch(symbol, timeframe, limit=limit)  # async
            return list(data or [])
        except TypeError:
            return list(fetch(symbol, timeframe, limit=limit) or [])  # sync
        except Exception:
            pass
    # 2) REST
...
def _csv_row(path: Path, headers: Sequence[str], row: Sequence[Any]) -> None:
    ...
    with path.open("a", encoding="utf-8") as f:
        ...
        vals = [str(v).replace(",", " ") if isinstance(v, str) else str(v) for v in row]
        f.write(",".join(vals) + "\n")
