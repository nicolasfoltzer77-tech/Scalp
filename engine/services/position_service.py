...
class PositionService:
    ...
    def _get(self, symbol: str) -> Position | None:
        return self.db.query(Position).filter_by(symbol=symbol).one_or_none()

    def apply_fill(self, symbol: str, side: str, price: float, amount: float, fee_cost: float = 0.0):
        pos = self._get(symbol)
        signed = D(str(amount)) if side == "buy" else -D(str(amount))
        ...
        if old_qty == 0:
            ...
            pos.entry_price = (D(str(pos.entry_price))*abs(old_qty) + px*abs(signed)) / abs(new_qty)
            pos.qty = new_qty
        else:
            ...
            pos.realized_pnl = D(str(pos.realized_pnl)) + pnl - fee
            pos.qty = new_qty
            if new_qty == 0:
                pos.entry_price = D("0")
