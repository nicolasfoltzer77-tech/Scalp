...
from engine.services.risk_engine import PositionSizer
...
class TpSlWatcher:
    ...
    def place_entry_and_tp(self, plan: TpSlPlan):
        o_entry = self.om.place(plan.symbol, plan.side, plan.entry_type, plan.amount, plan.entry_price,
                                reduce_only=False, signal_risk=plan.signal_risk)
        if plan.tp_price:
            tp_side = "sell" if plan.side == "buy" else "buy"
            self.om.place(plan.symbol, tp_side, "limit", plan.amount, plan.tp_price,
    ...
    def watch_and_stop(self, plan: TpSlPlan, *, poll_ms: int = 800, timeout_s: int = 600):
        ...
        while (time.time() - start) < timeout_s:
            ...
            if plan.side == "buy" and px <= plan.sl_price:
                self.om.place(plan.symbol, sl_side, "market", plan.amount, reduce_only=True,
                              signal_risk=plan.signal_risk)
                return True
...
def build_plan_from_env(adapter: CcxtBitgetAdapter) -> TpSlPlan:
    ...
    amt_env = os.getenv("AMOUNT")
    if amt_env:
        amount = float(amt_env)
    else:
        tier = int(os.getenv("RISK_TIER", "2"))
        amount, _ = PositionSizer(adapter).size_from_config(symbol, tier=tier)
    ...
    if (tp_pct and not tp_price_env) or (sl_pct and not sl_price_env):
        t = adapter.exchange.fetch_ticker(symbol)
