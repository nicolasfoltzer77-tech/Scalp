...
def _normalize_rows(raw: Iterable[Iterable[Any]]) -> List[List[float]]:
    """
    Retourne des lignes [ts, open, high, low, close, volume, quote_volume]
    Acceptes string/float/int, ordre Bitget habituel.
    """
    out: List[List[float]] = []
    for r in raw:
        # Certains endpoints renvoient 6 colonnes (sans quote_volume) -> inject 0.0
        if len(r) >= 7:
            ts, o, h, l, c, v, qv = r[:7]
        else:
            ts, o, h, l, c, v = r[:6]
    ...
    # Bitget renvoie souvent du plus récent -> plus ancien; on trie croissant
    out.sort(key=lambda x: x[0])
    return out

def _is_ok_dict(js: Dict[str, Any]) -> bool:
    code = str(js.get("code"))
    return code in ("00000", "0", "200")  # v1/v2 variantes

# ——— client ———

class BitgetClient:
    """
    Client OHLCV robuste (public) avec fallback v2 -> v1 et mix/spot.
    """
    ...
    def __init__(self, market: str = "umcbl", session: Optional[requests.Session] = None):
        """
        market:
          - 'umcbl'  : futures (USDT-Margined Perp)
          - 'spot'   : spot
        """
        ...
        # Routes candidates (on les teste dans l'ordre)
        if self.market == "umcbl":
            # mix (perp) v2 puis v1
            self.routes: List[Tuple[str, Dict[str, Any]]] = [
                ("/api/v2/mix/market/candles", {}),                 # v2
                ("/api/mix/v1/market/candles", {}),                 # v1
            ]
    ...
    def _get(self, path: str, params: Dict[str, Any]) -> Any:
        ...
        if r.status_code != 200:
            # Log paramétrage utile pour débug 400172
        ...
        # v2 peut renvoyer directement une liste (pas de 'code')
    ...
    def fetch_ohlcv(self, symbol: str, timeframe: str = "1m", limit: int = 200) -> List[List[float]]:
        """
        symbol:
          - umcbl : "BTCUSDT" (le suffixe _UMCBL sera ajouté si absent)
          - spot  : "BTCUSDT" (sans suffixe)
        """
        ...
        # Limites raisonnables; on segmente si > 200
        max_per_call = 200
        target = max(1, int(limit))
        chunks: List[List[float]] = []

        remaining = target
        last_ts: Optional[int] = None  # pour pagination si besoin
        ...
        while remaining > 0:
            ...
            # essaie les routes dans l’ordre (v2 -> v1)
            for path, extra in self.routes:
                params = dict(params_base)
                params.update(extra)
                # Pagination v2: param 'endTime' (ms) pour récupérer l'historique antérieur
                if last_ts:
                    # on recule d'1ms pour éviter le doublon
                ...
                try:
                    ...
                    if not rows:
                        # Essaie prochaine route si vide
                        last_err = BitgetError("Réponse vide")
                        continue
                    # garde pour pagination
                    last_ts = rows[0][0]
                    chunks.extend(rows)
                    break  # route OK
        ...
        # garde seulement le nombre demandé (en partant de la fin)
