...
class BitgetClient:
    ...
    def _ok(self, resp: requests.Response) -> dict:
        if 200 <= resp.status_code < 300:
            ...
            if code in ("00000", "success", 200, "200"):
                return js
            raise RuntimeError(f"HTTP {resp.status_code} payload error: {js}")
    ...
    def _symbol_for_market(self, pair: str) -> str:
        ...
        return f"{p}_UMCBL" if self.market == "umcbl" and not p.endswith("_UMCBL") else p
    ...
    def fetch_ohlcv(self, symbol: str, timeframe: str = "1m", limit: int = 1000) -> list[list[T.Union[str, float, int]]]:
        ...
        cap = MAX_LIMIT.get(self.market, 1000)
        limit = max(1, min(int(limit), cap))

        sym = self._symbol_for_market(symbol)
        if self.market == "umcbl":
            # essai 1 : candles
            params = {"symbol": sym, "granularity": tf_sec, "limit": limit}
            try:
                js = self._request("GET", self._route_mix_candles, params)
            except RuntimeError as e:
                # fallback vers history-candles (mêmes params)
                js = self._request("GET", self._route_mix_hist, params)
        else:
            # spot
            params = {"symbol": sym, "granularity": tf_sec, "limit": limit}
            js = self._request("GET", self._route_spot_candles, params)

        data = js.get("data") or []
        try:
            data = sorted(data, key=lambda r: int(r[0]))  # ts asc
        except Exception:
            pass
        return data
