...
from engine.backtest.run_multi import run_backtest_multi
...
def test_csv_multi_pairs(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("engine.strategy.generate_signal", simple_signal)
...
def test_paper_constraints(tmp_path, monkeypatch):
    make_csv(tmp_path, "BTC/USDT")
    monkeypatch.setattr("engine.strategy.generate_signal", tiny_qty_signal)
...
def test_seed_reproducible(tmp_path, monkeypatch):
    make_csv(tmp_path, "BTC/USDT")
    monkeypatch.setattr("engine.strategy.generate_signal", random_signal)
