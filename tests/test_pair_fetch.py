...
def test_fetch_pairs(monkeypatch, capsys):
    def fake_get(url, timeout=5):
        return _fake_resp(
            {
                "data": [
                    {"symbol": "AAA_USDT", "takerFeeRate": 0, "makerFeeRate": 0},
                    {"symbol": "AAA_USDC", "takerFeeRate": 0, "makerFeeRate": 0},
    ...
    importlib.reload(bc)
    capsys.readouterr()
    pairs = bc.fetch_pairs_from_mexc("http://example.com")
    assert pairs == ["AAA_USDT", "BBB_USDT", "BTC_USDT"]
    out, _ = capsys.readouterr()
    assert "Pairs: ['AAA_USDT', 'BBB_USDT', 'BTC_USDT']" in out
