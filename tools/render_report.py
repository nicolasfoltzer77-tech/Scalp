...
- À la fin, déclenche tools.publish_pages (copie JSON + health.json + push)

Ce fichier n’a pas besoin de lire les JSON côté Python :
tout est chargé par le navigateur depuis docs/data/*.json.
"""
...
function tickCountdown(){{countdown-=1;if(countdown<=0){{location.reload();return;}}document.getElementById('countdown').textContent='reload dans '+countdown+'s';}}
document.getElementById('btnRefresh').addEventListener('click',()=>location.reload());

function badgeStatusCell(st){{return `<td class="status-${{st}}">${{st}}</td>`;}}
function renderCounters(counts){{for(const k of ["MIS","OLD","DAT","OK"]){{const el=document.getElementById(k);el.textContent=`${{k}}: ${{counts[k]||0}}`;}}}}
function renderHeatmap(matrix,tf_list){{if(!matrix||!matrix.length){{document.getElementById('heatmap').innerHTML="<div class='muted'>Aucune matrice (status.json manquant).</div>";return;}}

  let html="<table><thead><tr><th>PAIR</th>"+tf_list.map(tf=>`<th>${{tf}}</th>`).join("")+"</tr></thead><tbody>";
  for(const row of matrix){{html+=`<tr><td><b>${{row.pair}}</b></td>`;for(const tf of tf_list){{const st=row[tf]||"MIS";html+=badgeStatusCell(st);}}html+="</tr>";}}
  html+="</tbody></table>";
...
function scoreRow(r){{const pf=+r.pf||0,mdd=+r.mdd||0,sh=+r.sharpe||0,wr=+r.wr||0;return pf*2+sh*0.5+wr*0.5-mdd*1.5;}}

function getFilters(){{const q=document.getElementById('filterPair').value.trim().toUpperCase();
...
function applyFilters(rows){{const f=getFilters();
  return rows.filter(r=>{{const okPair=!f.q||(r.pair||"").toUpperCase().includes(f.q);
...
function renderTopTable(allRows){{const rows=allRows.map(r=>Object.assign({{}},r,{{score:scoreRow(r)}}));
  ...
  for(const c of cols){{const cls=(sortState.col===c.key)?("sort-"+sortState.dir):"";html+=`<th data-col="${{c.key}}" class="${{cls}}">${{c.label}}</th>`;}}
  html+="</tr></thead><tbody>";
  sorted.slice(0,100).forEach((r,i)=>{{html+=`<tr><td>${{i+1}}</td><td>${{r.pair}}</td><td>${{r.tf}}</td><td>${{(+r.pf).toFixed(3)}}</td><td>${{((+r.mdd)*100).toFixed(1)}}%</td><td>${{r.trades||0}}</td><td>${{((+r.wr)*100).toFixed(1)}}%</td><td>${{(+r.sharpe).toFixed(2)}}</td><td>${{(+r.score).toFixed(2)}}</td></tr>`;}});
  html+="</tbody></table>";
  const wrap=document.getElementById('topTableWrap');wrap.innerHTML=html;
  wrap.querySelectorAll("th").forEach(th=>{{th.addEventListener("click",()=>{{const col=th.dataset.col;if(!col)return;
      sortState.col===col?(sortState.dir=(sortState.dir==="asc"?"desc":"asc")):(sortState.col=col,sortState.dir="desc");
      renderTopTable(allRows);
  }});}});
}}

async function loadJSON(path){{const url=path+"?_t="+Date.now();const r=await fetch(url);if(!r.ok)throw new Error("HTTP "+r.status+" on "+path);return await r.json();}}

async function refreshAll(){{
  setNow();countdown={AUTO_REFRESH_SECS};
  try {{
    ...
    if(Object.keys(health).length){{const st=(health.status||"").toLowerCase();
      const cls=st.includes("ok")?"health-ok":(st.includes("local")||st.includes("no-change")?"health-warn":"health-bad");
      hb.innerHTML=`<div>generated_at: <b>${{health.generated_at||"?"}}</b></div>
    ...
    document.getElementById("lastErrors").textContent=JSON.stringify(last,null,2);
...
setNow();refreshAll();setInterval(()=>{{countdown-=1;if(countdown<=0)location.reload();document.getElementById('countdown').textContent='reload dans '+countdown+'s';}},1000);
</script>
"""

def main():
    # 1) écrire le HTML dynamique
    ...
    # 2) publier → essai via module, sinon fallback vers chemin absolu
    try:
        subprocess.run([sys.executable, "-m", "tools.publish_pages"],
                       cwd=str(REPO_ROOT), check=True)
    except Exception as e1:
        print(f"[render] publish via -m indisponible ({e1}) → fallback fichier…")
        script = REPO_ROOT / "tools" / "publish_pages.py"
        try:
            subprocess.run([sys.executable, str(script)],
                           cwd=str(REPO_ROOT), check=True)
        except Exception as e2:
            print(f"[render] Publication ignorée: {e2}")
