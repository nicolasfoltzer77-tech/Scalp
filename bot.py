...
# -*- coding: utf-8 -*-
"""
Point d'entrée unique (scan / orchestrate) avec défauts raisonnables.
- Mode scan : offline via CSV (--csv) ou online via Bitget si présent.
- Mode orchestrate : boucle live Bitget.
"""
...
def _import_exchange():
    """
    Essaie d'importer le client Bitget. Retourne (classe, message_d'aide).
    """
    try:
    ...
    *, symbols: List[str], timeframe: str, cfg_path: str,
    ...
    for s in symbols:
        ...
        if not ohlcv:
            ...
            continue
        sig = signal_fn(
            symbol=s, timeframe=timeframe, ohlcv=ohlcv,
...
def parse_args() -> argparse.Namespace:
    ...
    # Valeurs par défaut pour éviter l'erreur si lancé sans args
...
def main():
    ...
    if not symbols:
        print("Aucun symbole fourni (utilise --symbols ou variable DEFAULT_SYMBOLS).")
        return
